...
const EditInvoicePage = observer(() => {
  ...
  useEffect(() => {
    ...
    //add cus name
    const invoiceCustomer = customers._customers.find(
  ...
  function removeInvoice(item) {
    ...
    if (item.temporary_id) {
      newInvoiceItems = invoiceItems.filter(
        (_item) => _item.temporary_id !== item.temporary_id
      );
    } else {
      newInvoiceItems = invoiceItems.filter((_item) => _item.id !== item.id);
    }
        ...
        <div className="row mt-3">
          ...
          <div className="col">
            <label
              ...
              style={{ marginTop: "8%", fontSize: "22px" }}
